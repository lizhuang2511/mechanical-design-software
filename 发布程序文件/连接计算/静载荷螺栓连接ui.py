...
class 静载荷轴向螺栓计算(HasTraits):
        ...
        def 计算应力2(self, event):
            self.残余预紧力=self.残余预紧力系数*self.轴向载荷*1000
            self.总载荷=self.残余预紧力+self.轴向载荷*1000
            self.预紧力=self.残余预紧力+(1-self.相对刚度)*self.轴向载荷*1000
            self.许用应力=self.螺栓屈服强度/self.安全系数
            self.计算应力=1.3*self.总载荷/((pi/4)*self.螺栓小径**2)
            return
        显示2 = Group(VGrid(Item('轴向载荷', label='轴向载荷(kn)'), Item('残余预紧力系数',label='残余预紧力系数'),
                          Item('螺栓大小', label='螺栓大小'), Item('螺栓小径',label='螺栓小径'),
                          Item('机械性能等级', label='机械性能等级'), Item('螺栓屈服强度',label='螺栓屈服强度'),
                          Item('相对刚度', label='相对刚度'), Item('连接材料',style='custom',label='连接材料'),
                          Item(label='-'),Item('安全系数', label='安全系数'), ),
                    Item('j计算', label='  '),
                    Item('预紧力', label='预紧力'),
        ...
        traits_view = View(显示2,
                           title='受轴向载荷紧螺栓计算-静载荷                         制作：lizhuang',
                           resizable=True,
                           width=1200,
                           height=600,
                           kind="live",
