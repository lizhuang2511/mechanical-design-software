...
class HelicalSpringDesignUI(HasTraits):
    ...
    def 显示材料(self, event):
        print('a')
        default_file_path = "..\弹簧设计\弹簧材料.pdf"
    ...
    def 显示推荐直径(self, event):
        print('a')
        default_file_path = "..\弹簧设计\钢丝选择.pdf"
        self.viewer = PdfViewer(default_file_path)
        self.viewer.show()
        return
    # 图形显示
    ...
    def 显示圈数计算方法(self, event):
        print('a')
        default_file_path = "..\弹簧设计\圈数和疲劳.pdf"
        self.viewer = PdfViewer(default_file_path)
        self.viewer.show()
        return
    ...
    def 类型选择(self, event):
        print('a')
        default_file_path = "..\弹簧设计\类型选择.pdf"
        self.viewer = PdfViewer(default_file_path)
        self.viewer.show()
        return
    # 图形显示
    ...
    viewid = Group(HGroup(
            VGroup(
                ...
                HGroup(VGroup(VGrid(
                    ...
                    springy=True
                    ),label='计算圈数'),layout='tabbed',style_sheet='*{font-size:25px}')
