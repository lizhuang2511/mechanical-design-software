...
def __khb__(pair):
    ...
    fsh = fm_b * 0.023 * (abs(1 + kp * factemp - 0.3) + 0.3) * ((b / d) ** 2)#这里有问题
    ...
    if fav == 1:
        ...
        fbx = 1.33 * b1 * fsh + b2 * fma#fbx有问题不一致

    ybone = __yb__(pair.gear_one, pair, fbx)
    ybtwo = __yb__(pair.gear_two, pair, fbx)
    yb = round(0.5 * (ybone + ybtwo), 1)
    fby = fbx - yb
    print('fm_b 98083/180=544',fm_b)
    print('b',b)
    print('fby',fby)
    if fby * cgammabeta / (2 * fm_b) >= 1:
        khb = sqrt((2. * fby * cgammabeta) / fm_b)#_KHbetaCalc1
    else:
        print('出发')
        khb = 1 + (fby * cgammabeta) / (2 * fm_b)#标准39页#这里源代码少除以了b齿宽
...
def __kfb__(pair):
    ...
    print(pair.gear_one.h,pair.gear_one.b)
...
class Bending(object):
    ...
    def calculate(self):
        ...
        return {
            'sigmafoone': sigmaf0one,
            'sigmafotwo': sigmaf0two,
            'sigmafone': sigmafone,
            'sigmaftwo': sigmaftwo,
